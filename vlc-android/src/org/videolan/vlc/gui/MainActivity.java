...
public class MainActivity extends SherlockFragmentActivity {
	...
	private static final String ADMOB_ID_INTERSTITIAL = "XXX";
	private static final String ADMOB_ID_BANNER = "XXX";
	...
	private boolean isFreeVersion() {
		return getResources().getBoolean(R.bool.is_free_version);
	}
	...
	private void showInterstitial() {
		...
		try {
			Log.d(TAG, "Load ad.");
			interstitial = new InterstitialAd(this, ADMOB_ID_INTERSTITIAL);
	...
	private void addBannerView(FrameLayout layout) {
		if (!isFreeVersion())
			return;

		adView = new AdView(this, AdSize.BANNER, ADMOB_ID_BANNER);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		{
			View wrapper = (ViewGroup) LayoutInflater.from(this).inflate(
					R.layout.aa_wrapper, null);

			v_main = LayoutInflater.from(this).inflate(R.layout.main,
					(FrameLayout) wrapper.findViewById(R.id.aa_w_frameLayout1));

			addBannerView((FrameLayout) wrapper
					.findViewById(R.id.aa_w_frameLayout2));
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (entry.id.equals("open_network_stream")) {
					mMenu.showContent();
					onOpenMRL();
					return;
				}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (itemId == R.id.ml_menu_sortby_name
				|| itemId == R.id.ml_menu_sortby_length) {
			Fragment current = getSupportFragmentManager().findFragmentById(
					R.id.fragment_placeholder);
			if (current == null)
		...
		} else if (itemId == R.id.ml_menu_about) {
			showSecondaryFragment("about");
		} else if (itemId == R.id.ml_menu_preferences) {
			intent = new Intent(this, PreferencesActivity.class);
			startActivityForResult(intent, ACTIVITY_RESULT_PREFERENCES);
		} else if (itemId == R.id.ml_menu_equalizer) {
			showSecondaryFragment("equalizer");
		} else if (itemId == R.id.ml_menu_refresh) {
		...
		} else if (itemId == R.id.ml_menu_last_playlist) {
			Intent i = new Intent(AudioService.ACTION_REMOTE_LAST_PLAYLIST);
			sendBroadcast(i);
		} else if (itemId == R.id.ml_menu_open_mrl) {
			onOpenMRL();
		} else if (itemId == R.id.ml_menu_search) {
			onSearchRequested();
			...
			if (mMenu.isMenuShowing())
				...
				mMenu.showMenu();
		} else if (itemId == R.id.search_clear_history) {
			((SearchFragment) fetchSecondaryFragment("search"))
					.clearSearchHistory();
		}
		return super.onOptionsItemSelected(item);
